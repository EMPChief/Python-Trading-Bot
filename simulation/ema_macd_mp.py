...
def run_pair(pair):
    ...
    pd.concat(trades).to_csv(f"./data/result/trades/macd_ema_trades_{pair}.csv")
...
def run_process(pair):
    ...
    results.to_csv(f"./data/result/macd_ema_res_{pair}.csv")
...
def run_ema_macd(ic: InstrumentCollection):
    ...
    pairs = get_sim_pairs(['USD', 'GBP', 'JPY', 'SEK', 'CHF', 'AUD', 'CAD', 'HKD'], ic)
